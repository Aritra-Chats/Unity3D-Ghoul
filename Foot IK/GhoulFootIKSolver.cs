...
public class EnemyFootIKSolver : MonoBehaviour
{
    #region Variables
    ...
    private void Update()
    {
        if(Physics.CheckSphere(transform.position, 0.2f, GroundLayer)) _isGrounded = true;
    ...
    private void OnAnimatorIK(int layerIndex)
    {
        ...
        if (_isGrounded) MovePelvisHeight();
        else if (!_isGrounded)
        {
